                ...
                try {
                    ...
                    for (int i = 0; i < numberOfUpdates; ++i) {
                        ...
                        try {
                            ...
                            boolean updateSuccessful = response.rowCount() > 0;

                            execute("select _primary_term, _seq_no from t where id = ?",
                                    new Object[]{partition.id}, TimeValue.timeValueSeconds(timeoutSeconds));
                            long primaryTerm = (long) response.rows()[0][0];
                            long seqNo = (long) response.rows()[0][1];
                            ...
                            if (updateSuccessful) {
                                ...
                                if (version.compareTo(partition.latestSuccessfulVersion()) <= 0) {
                                    historyResponse.accept(new CASFailureHistoryOutput(historyOutput.outputVersion));
                                }
                            }
